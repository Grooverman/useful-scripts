### Script to monitor a specific list of tickets on Jira

Whenever you run this script it will:
1. execute the query specified on the "config" file in the same directory, 
...
then you can add this script to your crontab so it's run every 5 minutes, for example:
```bash
*/5 * * * * /path/to/this/repo/get-jira-issues/get-jira-issues.sh
```
